...
export default async function analyzeProfile(id: string) {
    ...
    return [
      {
        ...
        value: `Holm Host is unable to process this spark profile. It appears that the platform is not supported for analysis. Platform: ${platform}`,
      },
    ];
  }
  ...
  if (cpu <= 2) fields.push({ name: '❌ Threads', value: `You only have ${cpu} thread(s).`, buttons: [{ text: 'Find a better host', url: 'https://racnhosting.com/' }] });
