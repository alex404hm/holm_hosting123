...
export default async function analyzeTimings(id: string) {
  ...
  try {
    ...
    return [{ name: '❌ Processing Error', value: 'Holm Host cannot process this spark profile. Please use an alternative spark profile.' }];
  }
  ...
  if (timing_cost > 300) {
    fields.push({ name: '❌ Timingcost', value: `Your timingcost is ${timing_cost}. Your cpu is overloaded and/or slow.`, buttons: [{ text: 'Find a better host', url: 'https://www.racnhosting.com' }] });
  }
  ...
  if (cpu <= 2) fields.push({ name: '❌ Threads', value: `You only have ${cpu} thread(s).`, buttons: [{ text: 'Find a better host', url: 'https://www.racnhosting.com' }] });
